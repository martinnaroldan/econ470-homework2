...
final.hcris <- rbind(final.hcris.v1996, final.hcris.v2010) %>%
  mutate(fy_end = mdy(fy_end), fy_start = mdy(fy_start),
         ...
         tot_discounts = parse_number(tot_discounts), hrrp_payment = parse_number(hrrp_payment),
         hvbp_payment = as.numeric(hvbp_payment), hrrp_payment = as.numeric(hrrp_payment),
         tot_charges = as.numeric(tot_charges), tot_operating_exp = as.numeric(tot_operating_exp),
         ip_charges = as.numeric(ip_charges), icu_charges = as.numeric(icu_charges),
         ancillary_charges = as.numeric(ancillary_charges), tot_discharges = as.numeric(tot_discharges),
         mcare_discharges = as.numeric(mcare_discharges), mcaid_discharges = as.numeric(mcaid_discharges),
         tot_mcare_payment = as.numeric(tot_mcare_payment), secondary_mcare_payment = as.numeric(secondary_mcare_payment)) %>%
...
unique.hcris3 <- duplicate.hcris2 %>%
...
duplicate.hcris3 <- duplicate.hcris3 %>%
